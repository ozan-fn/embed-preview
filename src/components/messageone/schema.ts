...
export const embedSchema = z
    ...
    .superRefine((data, ctx) => {
        if (!data.description && !data.title && !data.author && !data.provider && !data.footer && !data.fields.length && !data.image && !data.thumbnail) {
            ctx.addIssue({
...
export const componentButtonStyleSchema = z.literal(1).or(z.literal(2)).or(z.literal(3)).or(z.literal(4)).or(z.literal(5));

export type MessageComponentButtonStyle = z.infer<typeof componentButtonStyleSchema>;
...
export type MessageComponentSelectMenuOption = z.infer<typeof componentSelectMenuOptionSchema>;
...
export type MessageComponentSelectMenu = z.infer<typeof componentSelectMenuSchema>;
...
export type MessageComponentActionRow = z.infer<typeof componentActionRowSchema>;
...
export type MessageComponentTextDisplay = z.infer<typeof componentTextDisplaySchema>;
...
export type MessageComponentThumbnail = z.infer<typeof componentThumbnailSchema>;

export const componentAccessorySchema = z.union([componentThumbnailSchema, componentButtonSchema]);

export type MessageComponentAccessory = z.infer<typeof componentAccessorySchema>;
...
export type MessageComponentMediaGalleryItem = z.infer<typeof componentMediaGalleryItemSchema>;
...
export type MessageComponentMediaGallery = z.infer<typeof componentMediaGallerySchema>;
...
export type MessageComponentSeparator = z.infer<typeof componentSeparatorSchema>;

export const componentContainerSubComponentSchema = z.union([componentActionRowSchema, componentTextDisplaySchema, componentSectionSchema, componentMediaGallerySchema, componentSeparatorSchema, componentFileSchema]);

export type MessageComponentContainerSubComponent = z.infer<typeof componentContainerSubComponentSchema>;
...
export type MessageComponentContainer = z.infer<typeof componentContainerSchema>;

export const componentSchema = z.union([componentActionRowSchema, componentButtonSchema, componentSelectMenuSchema, componentSectionSchema, componentTextDisplaySchema, componentThumbnailSchema, componentMediaGallerySchema, componentFileSchema, componentSeparatorSchema, componentContainerSchema]);
...
export const webhookUsernameSchema = z.optional(
    z
        .string()
        .max(80)
        .refine((val) => !val.toLowerCase().includes("clyde") && !val.toLowerCase().includes("discord"), "Username can't contain 'clyde' or 'discord'")
        .refine((val) => val.toLowerCase() !== "everyone" && val.toLowerCase() !== "here", "Username can't be 'everyone'  or 'here'"),
);
