...
import type { Message, MessageEmbed, EmbedField, MessageComponentActionRow, MessageComponentButton, MessageComponentSelectMenu } from "./schema";
// @ts-expect-error markdown.js may not have proper types
import { toHTML } from "./markdown";
import { colorIntToHex } from "./discordUtil";
...
export default function MessagePreview({ msg }: { msg: Message }) {
    ...
    const defaultUsername = "Embed Generator";
    const defaultAvatarUrl = getRelativeUrl("/logo.svg");
                    ...
                    <div className="discord-message-inner">
                        ...
                        <div className="discord-message-content">
                            <span className="discord-author-info">
                                <span className="discord-author-username">{msg.username || defaultUsername}</span>
                            ...
                            <div className="discord-message-compact-indent">
                                {msg.embeds &&
                                    msg.embeds.map((embed: MessageEmbed) => {
                                                ...
                                                <div className="discord-embed-root">
                                                    <div className="discord-embed-wrapper">
                                                        <div className="discord-embed-grid">
                                                            {!!embed.provider?.name && <div className="discord-embed-provider overflow-hidden break-all">{embed.provider.url ? <a href={embed.provider.url}>{embed.provider.name}</a> : embed.provider.name}</div>}
                                                            {!!embed.author?.name && (
                                                                <div className="discord-embed-author overflow-hidden break-all">
                                                                    ...
                                                                    {embed.author.url ? <a href={embed.author.url}>{embed.author.name}</a> : embed.author.name}
                                                                </div>
                                                            )}
                                                            {!!embed.title && (
                                                            ...
                                                            {!!embed.fields.length && (
                                                                <div className="discord-embed-fields">
                                                                    {embed.fields.map((field: EmbedField) => (
                                                                        <div key={field.id} className={`discord-embed-field${field.inline ? ` discord-embed-inline-field discord-embed-inline-field-${(inlineFieldIndex++ % 3) + 1}` : ""}`}>
                                                                            <div
                                                            ...
                                                            {(embed.footer?.text || embed.timestamp) && (
                                                                <div className="discord-embed-footer overflow-hidden break-all">
                                                                    {embed.footer?.icon_url && <img src={embed.footer?.icon_url} alt="" className="discord-footer-image" />}
                                                                    {embed.footer?.text}
                                                                    {embed.footer?.text && embed.timestamp && <div className="discord-footer-separator">•</div>}
                                ...
                                <div className="discord-attachments">
                                    {msg.components &&
                                        msg.components
                                            ...
                                            .map((row: MessageComponentActionRow) => (
                                                <div className="discord-action-row" key={row.id}>
                                                    {row.components.map((comp) =>
                                                        comp.type === 2
                                                            ? (() => {
                                                                  const buttonComp = comp as MessageComponentButton;
                                                                  return buttonComp.style === 5 ? (
                                                                      <a className={`discord-button discord-button-hoverable discord-button-secondary ${buttonComp.disabled ? "discord-button-disabled" : ""}`} key={buttonComp.id} target="_blank" href={buttonComp.url} rel="noreferrer">
                                                                          {buttonComp.emoji &&
                                                                              (buttonComp.emoji.id ? (
                                                                                  <img src={`https://cdn.discordapp.com/emojis/${buttonComp.emoji.id}.${buttonComp.emoji.animated ? "gif" : "png"}`} alt="" className="discord-button-emoji" />
                                                                              ) : (
                                                                                  <Twemoji
                                                                                      options={{
                                                                                  ...
                                                                                  >
                                                                                      {buttonComp.emoji.name}
                                                                                  </Twemoji>
                                                                              ))}
                                                                          <span>{buttonComp.label}</span>
                                                                          <svg className="discord-button-launch" aria-hidden="false" width="16" height="16" viewBox="0 0 24 24">
                                                                      ...
                                                                      <div className={`discord-button discord-button-hoverable ${buttonColors[buttonComp.style]} ${buttonComp.disabled ? "discord-button-disabled" : ""}`} key={buttonComp.id}>
                                                                          {buttonComp.emoji &&
                                                                              (buttonComp.emoji.id ? (
                                                                                  <img src={`https://cdn.discordapp.com/emojis/${buttonComp.emoji.id}.${buttonComp.emoji.animated ? "gif" : "png"}`} alt="" className="discord-button-emoji" />
                                                                              ) : (
                                                            ...
                                                            : comp.type === 3
                                                              ? (() => {
                                                                    const selectComp = comp as MessageComponentSelectMenu;
                                                                    return (
                                                                        <div className={`discord-select-menu discord-select-menu-hoverable ${selectComp.disabled ? "discord-select-menu-disabled" : ""}`} key={selectComp.id}>
                                                                            <span className="discord-select-menu-placeholder">{selectComp.placeholder || "Make a selection"}</span>
                                                                            <svg className="discord-select-menu-icon" aria-hidden="true" role="img" width="24" height="24" viewBox="0 0 24 24">
                ...
                {responses.map((resp) => (
                    <div className="discord-message discord-highlight-ephemeral" key={resp.id}>
                        <div className="discord-replied-message">
                            <img src={msg.avatar_url || getRelativeUrl("/logo.svg")} alt="" className="discord-replied-message-avatar" />
                            <span className="discord-application-tag">Bot</span>
                            <span className="discord-replied-message-username">{msg.username || "Embed Generator"}</span>
                            <div className="discord-replied-message-content truncate">{msg.content || <span className="italic">Click to see attachment</span>}</div>
                        </div>
                        <div className="discord-message-inner">
                            <div className="discord-author-avatar">
                            ...
                            <div className="discord-message-content">
                                <span className="discord-author-info">
                                    ...
                                    <span className="discord-application-tag">
                                        <svg className="discord-application-tag-verified" aria-label="Verified Bot" aria-hidden="false" width="16" height="16" viewBox="0 0 16 15.2">
                                ...
                                <div className="discord-message-compact-indent">
                                    <div className="discord-message-ephemeral flex items-center">
                                        <svg className="discord-message-ephemeral-icon" aria-hidden="false" width="16" height="16" viewBox="0 0 24 24">
                                        ...
                                        <span className="discord-message-ephemeral-link" onClick={() => setResponses(responses.filter((r) => r.id !== resp.id))}>
                                            Dismiss message
                                        </span>
                                    </div>
                                </div>
